...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleSuggestionClick(suggestionText) {
        ...
        // Send all suggestions to backend
    ...
    function displayBooks(books) {
        ...
        books.forEach((book, index) => {
            ...
            // Consistent, sanitized description handling
            let processedDescription = 'No description available.';
            if (book.description) {
                const tempDiv = document.createElement('div');
                tempDiv.innerHTML = book.description; // Parse potentially unsafe HTML
                const textContent = tempDiv.textContent || tempDiv.innerText || "";
                processedDescription = textContent.length > 150 ? textContent.substring(0, 150) + '...' : textContent;
            }
