...
3.  **Vercel Configuration (`vercel.json`):**
    *   The `vercel.json` file in the root directory handles the build and routing configuration. It tells Vercel:
        *   How to build the Python API from `backend/main.py`.
        *   How to serve the `frontend/script.js` file statically.
        *   How to route requests starting with `/api/` to the Python backend.
        *   How to route requests for `/script.js` to the correct frontend file.
